...
describe("Realtime queue", () => {
  ...
  function client(
    ...
    return {
      id,
      handshake: {
        auth: { token: authToken, boardToken },
        address: "127.0.0.1",
      },
      data: {},
  ...
  beforeEach(() => {
    ...
    queries.snapshot.mockResolvedValue({
      scope: "DOCTOR",
      doctorId,
    ...
    queries.publicSnapshot.mockResolvedValue({
      scope: "PUBLIC",
      date: ticket.queueDate,
      items: [toPublicQueueTicket(ticket)],
    });
    queries.ticket.mockResolvedValue(ticket);
    boardTokens.verify.mockReturnValue({
      tokenId: "board-1",
  ...
  it("signals an expired session on sync", async () => {
    const socket = client();
    await gateway.handleConnection(socket as unknown as Socket);
    sessions.authenticate.mockRejectedValue(
  ...
  it("classifies only a verified expired access token as expired", async () => {
    ...
    await expect(
      realSessions.authenticate("invalid.token.value"),
  ...
  it("publishes an internal event to staff and a redacted event to the public room", async () => {
    ...
    const events = new QueueEventsService(
      queries as unknown as QueueQueryService,
      gateway,
    );
    ...
    expect(gateway.server.to).toHaveBeenNthCalledWith(
      2,
      QUEUE_DOCTOR_ROOM(doctorId),
    );
    ...
    const publicCall = emit.mock.calls.find(
      ([name]) => name === QUEUE_PUBLIC_STATUS_CHANGED_EVENT,
    );
    expect(publicCall?.[1]).toEqual({
      ...
      ticket: {
        ...
        maskedPatientName: "Pati***",
        status: ticket.status,
        queueNumber: ticket.queueNumber,
        queueDate: ticket.queueDate,
      },
    });
    expect(JSON.stringify(publicCall?.[1])).not.toContain(
      ticket.appointmentCode,
    );
    expect(JSON.stringify(publicCall?.[1])).not.toContain(ticket.appointmentId);
    expect(publicCall?.[1].ticket.maskedPatientName).not.toBe(
      ticket.patientName,
    );
  });
  ...
  it("still publishes to the doctor room if reception publish fails", async () => {
    ...
    gateway.server = {
      to: jest.fn((room: string) => {
        if (room === QUEUE_RECEPTION_ROOM)
          throw new Error("reception room failed");
    ...
    expect(doctorEmit).toHaveBeenCalledWith(
      APPOINTMENT_STATUS_CHANGED_EVENT,
  ...
  it("queries the doctor queue by date, active status, and doctor identity", async () => {
    ...
    database.getRepository.mockReturnValue({
    ...
    expect(snapshot).toEqual(
      expect.objectContaining({ scope: "DOCTOR", doctorId, items: [] }),
    );
